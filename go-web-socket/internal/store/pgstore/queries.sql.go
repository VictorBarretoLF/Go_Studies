...
const markAsAnswered = `-- name: MarkAsAnswered :exec
UPDATE messages
SET
    answered = true
WHERE
...
func (q *Queries) MarkAsAnswered(ctx context.Context, id uuid.UUID) error {
	_, err := q.db.Exec(ctx, markAsAnswered, id)
	return err
}
